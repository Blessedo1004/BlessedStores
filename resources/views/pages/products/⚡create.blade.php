...
new class extends Component
{
    ...
    public function save(){
        ...
        return DB::transaction(function () {
            ...
            $path = $this->cover_image->store('product-images','public');
...
<div class="dashboard-content">
        @php
            $rateLimitErrors = collect($errors->messages())
                ->except(['name', 'quantity', 'price','description', 'cover_image', 'product_images','store_id'])
                ->flatten();
        @endphp 
            ...
            <div class="card-body p-4 p-md-5">
                ...
                <form wire:submit="save" class="needs-validation" novalidate>
                    ...
                    <div class="row g-3 justify-content-center">
                        ...
                        <div class="col-12 col-md-6">
                            <label class="form-label fw-semibold text-dark small mb-2">Cover Image</label>
                            <input type="file" wire:model="cover_image" accept="image/*" required>
                            @error('cover_image')
                                <div class="invalid-feedback mt-1 d-block">{{ $message }}</div>
                            @enderror
                            @if($cover_image)
                                <div class="mt-2" wire:transition>
                                    <img src="{{ $cover_image->temporaryUrl() }}" alt="Cover Image Preview" class="rounded" style="height: 80px; max-width: 100%;">
                                </div>
                            @endif

                            <div class="mt-2" wire:loading wire:target="cover_image">
                                    Uploading...
                            </div>
                        </div>
                            ...
                            <div class="col-12">
                                <div class="product-image-previews mt-3" aria-label="Selected product images">
                                    @foreach ($product_images as $product_image)
                                        @php($image = is_array($product_image) ? $product_image[0] : $product_image)

                                        <div class="product-image-preview" wire:key="image-{{ $loop->index }}" wire:transition>
                                            <img src="{{ $image->temporaryUrl() }}" alt="Product image {{ $loop->iteration }} preview">
