...
new class extends Component
{
    ...
    public function with() {
        $query = Category::when($this->searchTerm, function ($query) {
        ...
        $categories = (clone $query)->orderBy('parent_id')
        ->orderBy('name')->paginate(10)->onEachSide(0);

        $count = (clone $query)->count();
        return compact('count','categories');
     }
    ...
    public function delete($slug){
        ...
        // Rate limiting
        $key = 'delete-category:' . $category->id . ':' . request()->ip();
...
<div>
        ...
        <div class="row d-flex align-items-center justify-content-between mb-4">
            <div class="col-12 col-lg-4 text-center text-lg-start">
                <h5 class="fw-bold text-dark mb-1">Categories</h5>
            ...
            <div class="col-12 col-lg-8">
                <div class="row d-flex justify-content-center justify-content-lg-end">
                    <div class="col-12 col-md-6 mt-4 mt-lg-0">
                        <input type="search" class="header-search-bar mx-auto d-block" placeholder="Search categories by name" wire:model.live.debounce.500ms="searchTerm" inputmode="search">
                    </div>

                    <div class="col-12 col-md-6 mt-4 mt-lg-0 text-center">
                        <a class="fill-btn border-0" href="{{ route('categories.create') }}"  wire:navigate>                        
                            <span class="fill-btn-inner">
                                <span class="fill-btn-normal">Add Category</span>
                                <span class="fill-btn-hover">Add Category</span>
                            </span>
                        </a>
                    </div>
                </div>
                ...
                <table class="table custom-table mb-0 w-100">
                    ...
                    <tbody>
                        ...
                        <tr wire:key="category-{{ $category->id }}" wire:transition>
                            <td class="fw-semibold text-dark">{{ Str::limit($category->name, 30) }}</td>
                            <td class="fw-semibold text-dark">{{ $category->parent?->name }}</td>
                            <td>
                                <div class="d-flex flex-column flex-sm-row align-items-center gap-2">
                                    <a href="{{ route('categories.edit'  , $category->slug) }}" class="btn btn-outline-secondary btn-sm rounded-pill" wire:navigate>Edit</a>
                                    <button class="btn btn-outline-danger btn-sm rounded-pill" wire:click="delete(@js($category->slug))" wire:confirm="Are you sure you want to delete this category?? This is a permanent action." wire:loading.attr="disabled">Delete</button>
                                </div>
                            </td>
                            </tr>  
                        @empty
