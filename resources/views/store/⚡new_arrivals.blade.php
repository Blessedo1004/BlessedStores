...
new class extends Component
{
    ...
    public function addToCart($slug, ?int $variantId = null, int $quantity = 1){
        $product = Product::with('productImages', 'productVariants.size')->where('slug' , $slug)->firstOrFail();
    ...
    public function showQuickView($slug){
        ...
        $this->quickViewProduct = Product::with('productImages', 'brand', 'categories', 'productVariants.size')
            ->where('slug', $slug)
            ->firstOrFail();
    ...
    public function selectQuickViewVariant(int $variantId){
    ...
    public function incrementQuickViewQuantity(){
        $variant = $this->selectedVariantId ? $this->quickViewProduct?->productVariants->firstWhere('id', $this->selectedVariantId) : null;
        $max = $variant ? $variant->quantity : ($this->quickViewProduct?->quantity ?? 1);

        if ($this->quickViewQuantity < $max) {
            $this->quickViewQuantity++;
        }
    }
};
                ...
                <div class="product-modal">
                    <div class="product-modal-wrapper p-relative">
                        ...
                        <div class="modal__inner">
                            @if($quickViewProduct)
                                @php
                                    $selectedVariant = $quickViewProduct->productVariants->firstWhere('id', $this->selectedVariantId);
                                    $quickViewPrice = $selectedVariant ? $selectedVariant->price : $quickViewProduct->price;
                                    $quickViewAvailable = $selectedVariant ? $selectedVariant->quantity : $quickViewProduct->quantity;
                                @endphp
